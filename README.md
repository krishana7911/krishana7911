### Hi there, I'm Krishana Gautam 👋

I'm a **Full-Stack Developer** passionate about building **scalable, high-performance web applications** and optimizing complex systems. I specialize in **PHP, JavaScript, Next.js, WordPress, API integrations, and CI/CD pipelines**. I’ve worked on **large-scale projects** for clients in **the US, India, and Europe**, improving performance, accessibility, and security.

<p align="left" style="display: flex; align-items: center; gap: 10px; font-size: 18px; font-weight: bold;">
  Let's Connect:
  <a href="https://www.linkedin.com/in/krishana-gautam/" target="_blank">
    <img src="https://skillicons.dev/icons?i=linkedin" alt="LinkedIn" width="25" />
  </a>
  <a href="mailto:krishanagautam7911@gmail.com">
    <img src="https://skillicons.dev/icons?i=gmail" alt="Gmail" width="25" />
  </a>
</p>


---

### 🔥 **Tech Stack**
<p align="left">
  <img src="https://skillicons.dev/icons?i=nextjs,react,nodejs,py,go,php,wordpress,sass,tailwind,d3,mysql,graphql,prisma,npm,pnpm,vite,redux,regex,redis,githubactions,aws,docker,webpack,gulp,elasticsearch,figma,git,github" />
</p>


---

### 📈 **GitHub Stats**
<p align="left">
  <img src="https://github-readme-stats.vercel.app/api?username=krishana7911&show_icons=true&theme=tokyonight&hide_border=true" alt="GitHub Stats" height="160" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=krishana7911&theme=tokyonight&hide_border=true" alt="GitHub Streak" height="160" />
</p>

### 🌟 **Most Used Languages**
<p align="left">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=krishana7911&layout=compact&theme=tokyonight&hide_border=true" alt="Most Used Languages" height="160" />
</p>
---

### 🚀 **Notable Work & Experience**
- Built and optimized high-traffic platforms, including a digital newspaper system handling heavy databases and complex queries.
- Led the revamp of slow & broken websites, improving design, accessibility, and performance.
- Implemented GDPR-compliant features for an educational platform in Europe, ensuring security and accessibility.
- Developed projects using monorepos with T3 stack, PostgreSQL, and ShadCN, improving maintainability and scalability.
- Enhanced sites with PWA capabilities & performance optimizations, reducing load times and improving UX.
- Contributed to R&D, project structuring & planning, working with clients in news, education, automobile, conservation, and logistics industries.

---
###  Beyond Code  
- 🏗️ **Systematic Problem-Solver** – I love breaking down complex issues into simple, actionable solutions, whether it's debugging code or optimizing workflows.  
- 🔍 **Curious & Always Learning** – From new tech stacks to better dev practices, I'm always exploring and improving.  
- 🏆 **Ownership Mentality** – I take pride in delivering quality work, ensuring projects are scalable, secure, and well-structured.  
- 🤝 **Collaborative & Adaptable** – Having worked with global teams, I understand the importance of clear communication & teamwork.  
- 🎨 **Eye for UI/UX** – I enjoy improving user experience, ensuring designs are not just functional but also visually appealing.  
- 📢 **Technical Mentor & Contributor** – I like helping peers debug, simplify problems, and share what I learn.  


---

### 🤝 Interpersonal Skills  
- **Effective Problem-Solving** – I break down complex challenges into structured, actionable solutions.  
- **Ownership & Accountability** – I take initiative, ensuring projects are well-structured, scalable, and secure.  
- **Clear Communication** – Whether it's collaborating with devs, designers, or stakeholders, I ensure transparency and clarity.  
- **Adaptability & Continuous Learning** – I thrive in fast-changing environments and stay updated with the latest industry trends.  
- **Collaboration & Mentorship** – I enjoy knowledge-sharing, helping peers debug, and structuring better workflows.  
- **User-Centric Thinking** – I focus on performance, accessibility, and UI/UX improvements to enhance user experience. 
 
---

### 🎯 **Fun Facts**
- 🎨 I love sketching and doing live location sketches.  
- ✈️ I’m an avid traveler & have explored multiple cities on solo trips

---

💡 *Always open to discussions, collaborations, and new opportunities in the dev community!*  
⭐️ **Feel free to connect with me or explore my repos!** 🚀  
